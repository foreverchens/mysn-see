...
public class KlineChangeEventConfiguration {
	...
	public ChangeEventFunc day3Event() {
		return (curVal, statisticM) -> {
			...
			return ChangeEventInfo.builder().symbol(statisticM.getSymbol())
								  .dataType(dataType)
								  .period(IntervalEnum.d3.name())
								  .location(location).curV(curVal).lowV(day3LowV).highV(day3HighV).eventTime(DateUtil.formatDateTime(new Date())).build();
		};
	}
